...
//var berryMasterAddress = '0x0Ba45A8b5d5575935B8158a88C631E9F9C95a2e5';
//var accountFrom = '0xe010ac6e0248790e08f42d5f697160dedf97e024';

console.log(_UTCtime);
console.log("Rinkeby address: ", berryMasterAddress);
...
module.exports =async function(callback) {
    ...
    let berryMaster = await BerryMaster.at(berryMasterAddress);  
        try{
             ...
             let ttbalanceNow = await berryMaster.balanceOf(accountFrom);
             //console.log('Berry Tributes balance', web3.utils.hexToNumberString(ttbalanceNow));
             let tributesBal =  await web3.utils.fromWei(ttbalanceNow,'ether');
             console.log('Berry Tributes balance', tributesBal); 

    /*****Get BTC last request*******/
    let btcCount = await berryMaster.getNewValueCountbyRequestId(2);
    let btc = await berryMaster.getTimestampbyRequestIDandIndex(2, btcCount - 1); //will this work with a zero index? (or insta hit?)
    //console.log("btc", web3.utils.hexToNumberString(btc))
    let btcDat =  await berryMaster.retrieveData(2, btc);
   ...
   //console.log(dt); // Gives Tue Mar 22 2016 04:00:00 GMT+0530 (IST)
    ...
    let ethCount = await berryMaster.getNewValueCountbyRequestId(1);
    let eth = await berryMaster.getTimestampbyRequestIDandIndex(1, ethCount - 1); //will this work with a zero index? (or insta hit?)
    let ethDat =  await berryMaster.retrieveData(1, eth);
    ...
    if (gasP != 0) {
        try{
            var oracle = await new web3.eth.Contract(oracleAbi,berryMasterAddress);
        ...
        try{
            ...
            .on('transactionHash', function(hash){
                ...
                var ownerlink = "".concat('<https://rinkeby.etherscan.io/address/',berryMasterAddress,'>' );
